...
def connect():
    global engine
    try:
        connection_string = f"postgresql://{os.getenv('pablomorena')}:{os.getenv('contraseña')}@{os.getenv('localhost')}/{os.getenv('base_de_datos')}"
...
if conectar is None:
    exit() 
# 2) Create the tables
with conectar.connect() as connection:
...
with engine.connect() as connection:
    connection.execute(text("""INSERT INTO publishers (publisher_id, name) VALUES
        (1, 'O Reilly Media'),
        (2, 'A Book Apart'),
        (3, 'A K PETERS'),
        (4, 'Academic Press'),
...
df = pd.read_sql("SELECT * FROM publishers;", conectar)
print(df)
